<template>
  <div class="min-h-screen bg-gradient-to-b from-white to-sky-50/30 px-4 md:px-10 py-8">
    ...
    <EmptyState v-if="items.length === 0">
      {{ $t('cart.empty') }}
      <NuxtLink to="/products" class="text-sky-600 hover:underline">{{ $t('cart.shopNow') }}</NuxtLink>.
    </EmptyState>
    ...
    <div v-else class="grid md:grid-cols-3 gap-6">
      ...
      <div class="md:col-span-2 space-y-4">
        ...
        >
          ...
          <div class="flex items-center gap-2">
            ...
            <button
              class="h-9 w-9 rounded-lg border bg-gray-50 hover:bg-gray-100 transition"
              style="cursor: pointer;"
              @click="inc(idx)"
          ...
          <div class="w-28 text-right">
            <p class="font-semibold">{{ money(lineTotal(it)) }}</p>
            <button class="text-xs text-gray-500 hover:underline mt-1" @click="remove(idx)">
              {{ $t('cart.remove') }}
            </button>
          </div>
        </div>
      ...
      <!-- Summary -->
      <aside class="md:sticky md:top-20 h-max bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
        ...
        <NuxtLink to="/checkout">
          <BaseButton variant="primary" class="w-full py-3 text-base font-semibold shadow-md hover:shadow-lg" style="cursor: pointer;">
            {{ $t('cart.proceed') }}
          </BaseButton>
        </NuxtLink>
